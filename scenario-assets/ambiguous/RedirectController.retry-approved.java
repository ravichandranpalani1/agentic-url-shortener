 ...
 * docs/scenarios/03-ambiguous.md): the analytics submission itself is
 ...
 * dropped instead of the exception propagating out of the redirect handler.
 * This keeps the original async-by-design guarantee (redirect latency never
 * depends on the analytics write) while closing the one gap in it: an
 * executor-level rejection used to be able to surface as an uncaught
 * exception on the request thread.
 */
@RestController
